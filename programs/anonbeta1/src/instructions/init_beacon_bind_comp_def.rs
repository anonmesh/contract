...
#[init_computation_definition_accounts("beacon_bind", payer)]
#[derive(Accounts)]
pub struct InitBeaconBindCompDef<'info> {
    #[account(mut)]
...
pub(crate) fn handler(ctx: Context<InitBeaconBindCompDef>) -> Result<()> {
    init_comp_def(
        ctx.accounts,
