...
describe("ble-revshare", () => {
    ...
    it("Executes payment with Broadcaster (70/30)", async () => {
        ...
        try {
            ...
            // Check balances
            const recipientBalance = (await getAccount(provider.connection, recipientTokenAccount)).amount;
            const broadcasterBalance = (await getAccount(provider.connection, broadcasterTokenAccount)).amount;
